...
type Options struct {
	EnableScrapeTimeoutMetric bool
}
...
func (m *Manager) reload() {
	...
	for setName, groups := range m.targetSets {
		if _, ok := m.scrapePools[setName]; !ok {
			...
			sp, err := newScrapePool(scrapeConfig, m.append, m.jitterSeed, log.With(m.logger, "scrape_pool", setName), m.opts.EnableScrapeTimeoutMetric)
			if err != nil {
